...
#ifdef __linux__ /* function drm_get_dirty_info_name() does not appear to exist */
...
#else /* drm_drawable.c  has clip rects in DRM_MEM_DRAWABLE */
		clips = malloc(num_clips * sizeof(*clips), DRM_MEM_DRAWABLE, M_WAITOK | M_ZERO);
#endif
		if (!clips) {
			ret = -ENOMEM;
			goto out_err1;
